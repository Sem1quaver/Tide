...
$\frac{V_{tid}}{g}  =\sum_{n=2}^{\infty} \sum_{m=0}^{n} N_{n}^{m} P_{n}^{m}(\cos \theta)\left[a_{n}^{m}(t) \cos m \phi+b_{n}^{m}(t) \sin m \phi\right]$<br />$\begin{array}{l}
\begin{aligned}
...
#### 潮汐模型
$\begin{array}{c}
\frac{D \vec{u}}{D t}+2\left(\vec{u} \times \vec{f}_{c}\right)=-g \nabla_{h} \eta-\nabla_{h} V_{t i d}+A_{h} \nabla_{h}^{2} \vec{u}-\vec{F}, \\
\frac{\partial \eta}{\partial t}+\nabla_{h} \cdot \int_{-H+\eta_{b}}^{\eta} \vec{u} d z=0,
\end{array}\\
\vec{F}=\overleftrightarrow{C}_{t i d} \cdot \vec{u} / H+C_{D}|\vec{u}| \vec{u} / H$
## 球面浅水系统
### 代码分析
#### 浅水模型
```python
...
[shallow_water.gif](https://www.yuque.com/attachments/yuque/0/2024/gif/42656868/1708168604275-6cc0b5ef-2aef-47e6-884f-33e067838664.gif)
## 思考
核心问题：如何附加引潮势？
